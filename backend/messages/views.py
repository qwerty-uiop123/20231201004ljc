...
class ConversationListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        ).prefetch_related('participants', 'participants__user').order_by('-last_message_at')
...
def send_direct_message(request):
    ...
    if serializer.is_valid():
        ...
        ).filter(participants__user=recipient).first()
...
def unread_count(request):
    ...
    unread_messages = Conversation.objects.filter(
        participants__user=request.user
    ).aggregate(total_unread=models.Sum('participants__unread_count'))
